/**
 * Build dist/ from src/coriolis.js.
 *
 *   dist/coriolis.min.js     the whole library, every body
...
const SRC = path.join(ROOT, 'src', 'coriolis.js');
...
function readSource() {
  ...
  if (!marks.length) throw new Error('no section banners found in src/coriolis.js');
...
function assertModesAreIndependent(modes) {
  ...
  if (problems.length) {
    throw new Error('modes must depend only on the core, but:\n' + problems.join('\n') +
      '\nMove the shared helper into the core section at the top of src/coriolis.js.');
  }
}
...
(async () => {
  ...
  const licence = `/*! coriolis ${version} | MIT | github.com/TonkaTuff/coriolis */\n`;

  // the body table, read straight out of the source so this never drifts
  const Coriolis = require(SRC);
  const bodies = {};
  for (const [name, b] of Object.entries(Coriolis.BODIES)) bodies[name] = b;
  const canonical = new Set(Object.values(Coriolis.GROUPS).flat());
  ...
  const rows = [];
  rows.push(['coriolis.min.js', `all ${canonical.size}`, await squeeze(full, 'coriolis.min.js')]);
  ...
  console.log(`coriolis ${version} -> dist/  (${rows.length} files)\n`);
